...
class voxelization(nn.Module):
    ...
    def forward(self, data_dict):
        ...
        for idx, scale in enumerate(self.scale_list):
            ...
            unq = torch.cat([unq[:, 0:1], unq[:, [3, 2, 1]]], dim=1)   #xyz --> zyx
...
class voxel_3d_generator(nn.Module):
    ...
    def prepare_input(self, point, grid_ind, inv_idx):
        pc_mean = torch_scatter.scatter_mean(point[:, :3], inv_idx, dim=0)[inv_idx]    #calculate the mean value of each voxel
        ...
        voxel_centers = grid_ind * intervals + coors_range_xyz[:, 0].to(point.device)  #calculate the center coordinate of each voxel
    ...
    def forward(self, data_dict):
        ...
        #data_dict['sparse_tensor_scale_1'] = spconv.SparseConvTensor(
        #    features=features,
        ...
        data_dict['pt_fea'] = pt_fea
        data_dict['scale_1']['features'] = features
