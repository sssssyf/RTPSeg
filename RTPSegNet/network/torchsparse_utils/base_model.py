...
class LightningBaseModel(pl.LightningModule):
    ...
    def test_step(self, data_dict, batch_idx):
        ...
        pred_path = pred_path.replace('datasets','RTPSegNet_pred_0320')  ####
        if not os.path.exists(pred_path):
            os.makedirs(pred_path)
        pred = prediction.cpu().numpy().astype(np.uint8).reshape(-1,1)
        raw_labels = raw_labels.cpu().numpy().astype(np.uint8).reshape(-1,1)
        xyz = data_dict['points'].F[:,:3].cpu().numpy()#.astype(np.uint8)
        xyz_pred = np.concatenate((xyz, raw_labels, pred),axis=1)
