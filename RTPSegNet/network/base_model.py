...
class LightningBaseModel(pl.LightningModule):
    ...
    def training_step(self, data_dict, batch_idx):
        ...
        #print(str(self.current_epoch)+'_'+str(batch_idx))
        #self.log('train/acc', self.train_acc, on_epoch=True,prog_bar=True)
        #self.log('train/loss', data_dict['loss'], on_epoch=True,prog_bar=True)
        self.log('train/loss_main_ce', data_dict['loss_main_ce'],on_step=True,prog_bar=True)
        self.log('train/loss_main_lovasz', data_dict['loss_main_lovasz'],on_step=True,prog_bar=True)
    ...
    def test_step(self, data_dict, batch_idx):
        indices = data_dict['indices']
        origin_len = data_dict['origin_len']
        raw_labels = data_dict['labels'].cpu()
        ...
        # pred_path = path.replace("lidar", "pred").split("/")  # save the prediction results
        # pred_npy = pred_path[-1]
        # pred_path = "/".join(pred_path[:-1])
        # if not os.path.exists(pred_path):
        #     os.makedirs(pred_path)
        # pred = prediction.cpu().numpy().astype(np.uint8)
        # raw_labels = raw_labels.cpu().numpy().astype(np.uint8)
        # xyz = data_dict['ref_xyz'].cpu().numpy()#.astype(np.uint8)
        # xyz_pred = np.concatenate((xyz, raw_labels.T, pred.T),axis=1)
        # np.save(pred_path+'/'+pred_npy, xyz_pred)
    ...
    def validation_epoch_end(self, outputs):
        ...
        self.log('val/mIoU', mIoU, self.current_epoch)
        self.log('val/best_miou', best_miou, self.current_epoch)
